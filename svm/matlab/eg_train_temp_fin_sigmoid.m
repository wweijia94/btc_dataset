...
test_num = 1000;
% costs = [ 0.0001 0.001 0.01 0.1 1 10 100 1000 10000];
costs = [ 0.0001 0.001 0.01 0.1 1 10 100 1000 ];
% costs = [ 10 ];

degree = [0 1 2 3 4 5 10];

% gamma = [ 0.001 0.01 0.1 1 10 300 10000];
% gamma = [ 1 ];
gamma = [-100 -10 -1 0 1 10 100];

test_errors = zeros([length(costs), length(gamma), length(ns), length(degree)]);
train_errors = zeros([length(costs), length(gamma), length(ns), length(degree)]);
d = 1;
...
for c_idx= 1:length(costs)
    ...
    s = sprintf('-s 3 -t 1 -d %d -c %d -r %d', deg, c, g);
...
%     title(sprintf('usd, %s, n=%d',s,n));
    ...
    test_errors(c_idx, g_idx, n_idx, d_idx) = test_error;
    train_errors(c_idx, g_idx, n_idx, d_idx) = train_error;
...
for n_idx = 1:length(ns)
    ...
    fprintf(sprintf('%f,\t%f,\t%d,\t%f,\t%f,\t%s\n', train_error, test_error,n,gamma(g_idx),c,s));
end
end
end
