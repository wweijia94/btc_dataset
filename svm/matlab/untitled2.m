...
m = 4500;   %number of training instances
...
% svm_model = libsvmtrain(train_labels, train_set, '-s 3 -t 2 -c 1 -g 0.001');    %RBF
% svm_model = libsvmtrain(train_labels, train_set, '-s 3 -t 0 ');    %linear
svm_model = libsvmtrain(train_labels, train_set, '-s 3 -t 3 ');    %sigmoid

[pred_labels, accuracy, prob_estimates] = libsvmpredict(test_labels, test_set, svm_model);

figure(100);
hold off;
plot(test_labels*max_price + mean_price);
hold on;
plot(pred_labels*max_price + mean_price);
legend('actual', 'prediction');
test_error = sum(((test_labels*max_price + mean_price)-(pred_labels*max_price + mean_price)).^2)/test_num