...
raw_prices = csvread('Gdax_BTCUSD_1h_new.csv', 2, 2);
ethraw_prices = csvread('Gdax_ETHUSD_1h_new.csv', 2, 2);
btclow_prices = raw_prices(:,3);
btchigh_prices = raw_prices(:,2);
...
%ethprices = zeros(size(ethprices));
...
for n_idx = 1:length(ns)
    n = ns(n_idx);
    [all_set, all_labels, all_maxes, all_means ] = series2features_mvar(ethprices, n, 1,1);
